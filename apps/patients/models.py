...
class Patient(models.Model):
    ...
    # Timestamp fields (for database compatibility)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.patient_id:
            self.patient_id = self.generate_patient_id(using_database=using)

        # Ensure datetime fields are timezone-aware
        from django.utils import timezone
        if self.last_visit and self.last_visit.tzinfo is None:
            self.last_visit = timezone.make_aware(self.last_visit)
    ...
    def generate_patient_id(self, using_database=None):
        """Generate unique patient ID for ZAIN HMS"""
        # Get last patient 
        last_patient = Patient.objects.order_by('-registration_date').first()
        ...
        return f"ZAIN-PAT-{new_number:06d}"
