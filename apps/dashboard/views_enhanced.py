...
class EnhancedDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def _get_admin_dashboard_data(self, hospital, today, yesterday, week_start, month_start):
        ...
        patients_today = Patient.objects.filter(hospital=hospital, created_at__date=today).count()
        patients_yesterday = Patient.objects.filter(hospital=hospital, created_at__date=yesterday).count()
        patients_this_week = Patient.objects.filter(hospital=hospital, created_at__date__gte=week_start).count()
        ...
        appointments_today = Appointment.objects.filter(hospital=hospital, date=today).count()
        ...
        # Staff Analytics
        active_doctors = Doctor.objects.filter(hospital=hospital, user__is_active=True).count()
        active_nurses = Nurse.objects.filter(hospital=hospital, user__is_active=True).count()
        total_staff = User.objects.filter(hospital=hospital, is_active=True).count()
    ...
    def _get_patient_demographics_data(self, hospital):
        ...
        # Gender distribution
        gender_data = Patient.objects.filter(hospital=hospital).values('gender').annotate(
            count=Count('id')
        )
