...
class AIClinicalDashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            clinical_engine = ClinicalDecisionEngine(str(self.request.tenant.id))
            ...
            # Get pending decision support recommendations (not yet implemented)
            ...
            high_risk_patients = Patient.objects.filter(
                tenant=self.request.tenant,
                clinical_alerts__status='ACTIVE'
            ).distinct().annotate(
                alert_count=Count('clinical_alerts')
    ...
    def _calculate_ai_metrics(self):
