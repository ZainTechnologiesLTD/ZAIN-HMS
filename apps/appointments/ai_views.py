...
class AISchedulingDashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            ai_scheduler = AIScheduler(str(self.request.tenant.id))
    ...
    def get(self, request, doctor_id=None):
        ...
        try:
            context = {
                ...
                'appointment_types': AppointmentType.objects.filter(tenant=request.tenant),
                'patients': Patient.objects.filter(tenant=request.tenant).order_by('full_name')[:100],
                'ai_enabled': True
            }
            
            if doctor_id:
                doctor = get_object_or_404(Doctor, id=doctor_id, tenant=request.tenant)
                context['selected_doctor'] = doctor
                
                # Get AI-optimized slots for today and next 7 days
                ai_scheduler = AIScheduler(str(request.tenant.id))
                optimized_schedule = self._get_optimized_schedule(ai_scheduler, doctor)
            ...
            messages.error(request, 'Unable to load AI scheduling interface.')
        ...
        try:
            with transaction.atomic():
                ...
                if appointment_type_id:
                    appointment_type = AppointmentType.objects.get(id=appointment_type_id, tenant=request.tenant)
