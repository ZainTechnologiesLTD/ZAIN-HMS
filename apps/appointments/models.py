...
class Appointment(models.Model):
    ...
    serial_number = models.CharField(max_length=20, unique=True, blank=True, help_text="Serial number for doctor's schedule slot")
    barcode = models.CharField(max_length=50, unique=True, blank=True, help_text="Barcode for easy scanning")
    
    # Participants
    patient = models.ForeignKey(Patient, on_delete=models.CASCADE, related_name='appointments')
    doctor = models.ForeignKey(Doctor, on_delete=models.CASCADE, related_name='appointments')
    department = models.CharField(max_length=100, blank=True, help_text="Department/Specialty")
    ...
    slot_time_start = models.TimeField(null=True, blank=True, help_text="Slot start time from doctor schedule")
    slot_time_end = models.TimeField(null=True, blank=True, help_text="Slot end time from doctor schedule")
    duration_minutes = models.PositiveIntegerField(default=30)
    ...
    patient_phone = models.CharField(max_length=20, blank=True, help_text="Patient phone for notifications")
    patient_email = models.EmailField(blank=True, help_text="Patient email for notifications")
    ...
    def save(self, *args, **kwargs):
        # Generate required fields if not present
        ...
        # Single database setup - use default database
    ...
    def generate_serial_number(self):
        """Generate serial number for doctor's schedule slot"""
        from datetime import datetime
        ...
        if self.doctor and self.appointment_date:
            ...
            # Count appointments for this doctor within the same slot window if provided
            qs = Appointment.objects.filter(
                doctor=self.doctor,
                appointment_date=self.appointment_date,
            ).exclude(pk=self.pk)

            if self.slot_time_start and self.slot_time_end:
                qs = qs.filter(
                    appointment_time__gte=self.slot_time_start,
                    appointment_time__lt=self.slot_time_end,
                )

            appointments_count = qs.count() + 1

            doctor_initials = ''.join([name[0].upper() for name in self.doctor.get_full_name().split() if name])[:3]
            date_str = self.appointment_date.strftime('%m%d')
            return f"{doctor_initials}-{date_str}-{appointments_count:03d}"
        return f"SRL-{uuid.uuid4().hex[:8].upper()}"
    
    def generate_barcode(self):
        """Generate unique barcode for appointment"""
    ...
    def clean(self):
        ...
        # Capacity-aware validation: enforce per-slot capacity, not unique time conflict
        if self.doctor and self.appointment_date and self.appointment_time:
            # Require slot window to validate capacity properly
            slot_start = self.slot_time_start
            slot_end = self.slot_time_end
            if slot_start and slot_end:
                ...
                # Try to get capacity from schedule if available via related model; fall back to >=1
                try:
                    ...
                    capacity = schedule.max_patients if schedule and getattr(schedule, 'max_patients', None) else 1
                except Exception:
                    capacity = 1

                if booked_count >= capacity:
                    raise ValidationError("Selected slot is full. Please pick another slot.")
    
    @property
    def date_time(self):
        """Return combined datetime object as property"""
        if self.appointment_date and self.appointment_time:
