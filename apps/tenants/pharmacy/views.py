...
from .models import (
    Medicine, MedicineStock, PharmacySale, PharmacySaleItem, 
    Prescription, PrescriptionItem, DrugCategory, Manufacturer
)
from .forms import PharmacySaleForm, MedicineSearchForm
...
def pharmacy_dashboard(request):
    ...
    context = {
        ...
        'low_stock_medicines': Medicine.objects.filter(
            medicinestock__quantity__lte=F('minimum_stock')
        ).distinct().count(),
        'expired_medicines': MedicineStock.objects.filter(
            expiry_date__lt=timezone.now().date()
        ).count(),
        'today_sales': PharmacySale.objects.filter(
            created_at__date=timezone.now().date()
        ).aggregate(total=Sum('total_amount'))['total'] or Decimal('0.00'),
        'pending_prescriptions': Prescription.objects.filter(is_fulfilled=False).count(),
    }
...
def medicine_list_view(request):
    ...
    medicines = medicines.annotate(
        total_stock=Sum('medicinestock__quantity')
...
def create_sale_view(request):
    ...
    if request.method == 'POST':
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            ...
            available_stock = MedicineStock.objects.filter(
                medicine=medicine
...
def fulfill_prescription_view(request, prescription_id):
    ...
    if request.method == 'POST':
        ...
        for prescription_med in prescription.items.all():
...
def stock_report_view(request):
    ...
    medicines = Medicine.objects.filter(is_active=True).annotate(
        total_stock=Sum('medicinestock__quantity'),
        expired_stock=Sum('medicinestock__quantity', filter=Q(medicinestock__expiry_date__lt=timezone.now().date()))
    ).select_related('category', 'manufacturer')
    
    # Filter options
    filter_type = request.GET.get('filter')
    if filter_type == 'low_stock':
        medicines = medicines.filter(total_stock__lte=F('minimum_stock'))
