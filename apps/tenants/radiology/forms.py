...
class RadiologyOrderForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if filter_param:
            ...
            self.fields['ordering_doctor'].queryset = Doctor.objects.filter(user_id__in=hospital_user_ids)
        else:
            ...
            self.fields['ordering_doctor'].queryset = Doctor.objects.using('default').none()
...
class ImagingStudyForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if hospital:
            self.fields['order_item'].queryset = RadiologyOrderItem.objects.filter(order__hospital=hospital)
