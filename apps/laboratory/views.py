...
from .forms import (
    ...
    LabSectionForm, SampleCollectionForm
)
...
class LabTestListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['categories'] = LabSection.objects.filter(is_active=True)
        context['search'] = self.request.GET.get('search', '')
        context['selected_category'] = self.request.GET.get('category', '')
        return context


class LabTestCreateView(UnifiedSystemMixin, CreateView):
    model = LabTest
...
# Test Category Views
class LabSectionListView(ListView):  # TenantFilterMixin temporarily commented:
    model = LabSection
    template_name = 'laboratory/lab_section_list.html'
    context_object_name = 'lab_sections'
    required_permissions = ['laboratory.view_labsection']
    # tenant_filter_field removed for unified single-DB mode


class LabSectionCreateView(UnifiedSystemMixin, CreateView):
    model = LabSection
    form_class = LabSectionForm
    template_name = 'laboratory/lab_section_form.html'
    success_url = reverse_lazy('laboratory:lab_section_list')
    required_permissions = ['laboratory.add_labsection']

    def form_valid(self, form):
        messages.success(self.request, f'Lab section "{form.instance.name}" created successfully.')
        return super().form_valid(form)
...
class LabOrderCreateView(UnifiedSystemMixin, CreateView):
    model = LabOrder
