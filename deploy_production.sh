...
python manage.py makemigrations --settings=zain_hms.production_settings
...
# Check for DEBUG=False in production settings
if grep -q "DEBUG = False" zain_hms/production_settings.py 2>/dev/null; then
    echo -e "${GREEN}✅ DEBUG disabled for production${NC}"
else
    echo -e "${RED}❌ DEBUG should be False in production${NC}"
fi

# Check for ALLOWED_HOSTS configuration
if grep -q "ALLOWED_HOSTS" zain_hms/production_settings.py 2>/dev/null; then
    echo -e "${GREEN}✅ ALLOWED_HOSTS configured${NC}"
else
    echo -e "${YELLOW}⚠️  Configure ALLOWED_HOSTS for production${NC}"
fi
